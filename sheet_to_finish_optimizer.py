...
def sheet_size_optimizer():
    ...
    sheet_unit = st.radio("Sheet Size Unit", ("Millimeters (mm)", "Inches (in)"), key="sheet_unit")
    finish_unit = st.radio("Finish Size Unit", ("Millimeters (mm)", "Inches (in)"), key="finish_unit")

    def to_mm(value, unit):
        return value * 25.4 if unit == "Inches (in)" else value

    # Sheet dimensions
    sw = st.number_input("Sheet Width", min_value=1.0, value=330.0, key="sw")
    sh = st.number_input("Sheet Height", min_value=1.0, value=483.0, key="sh")

    # Finish dimensions
    fw = st.number_input("Finish Width", min_value=1.0, value=210.0, key="fw")
    fh = st.number_input("Finish Height", min_value=1.0, value=297.0, key="fh")
